...
export default function NoticeBoard() {
  ...
  // Fetch notices from public/notices.json
  useEffect(() => {
    fetch('/notices.json')
      .then((res) => res.json())
      .then((data) => {
  ...
  // Limit pinned notices strictly to the TOP 2 as requested
  ...
  const currentRecentNotices = unpinnedNotices.slice(indexOfFirstItem, indexOfLastItem);
  ...
  // Category counts
  const categories = ['All', 'Academic', 'Examination', 'Administrative', 'Events'];
  const getCategoryCount = (cat) => {
    if (cat === 'All') return notices.length;
    return notices.filter((n) => n.category.toLowerCase() === cat.toLowerCase()).length;
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900/5 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:16px_16px] text-slate-800 font-sans pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
        ...
        {/* PINNED ANNOUNCEMENTS (EXACTLY 2 CARDS) */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Left Column: Categories Sidebar & Support Card */}
          ...
          <div className="lg:col-span-9">
            ...
            {loading ? (
              ...
              <div className="bg-white/70 backdrop-blur-md rounded-2xl p-12 text-center border border-slate-200/80 text-slate-500">
                ...
                <p className="text-xs text-slate-400 mt-1">Try resetting your search filter or category selection.</p>
              </div>
            ) : (
